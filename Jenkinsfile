...
stage('Build Image') {
	...
	steps {
        ...
        stage('Deploy - Stage') {
            environment {
                APP_NAMESPACE = "jygujn-shopping-cart-stage"
            }
            steps {
            sh """
		oc set image \
		deployment ${DEPLOYMENT_CONFIG_STAGE} \
		shopping-cart-stage=quay.io/yun1_kwon/do400-deployingenvironments:build-${BUILD_NUMBER} \
		...
		shopping-cart-production=quay.io/yun1_kwon/do400-deployingenvironments:build-${BUILD_NUMBER} \
		-n ${APP_NAMESPACE} --record
		"""		
		}
        }
